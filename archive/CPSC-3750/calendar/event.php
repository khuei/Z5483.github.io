...
<body>
  ...
  if ($_POST) {
    ...
    $insEvent_sql = "INSERT INTO calendar_events (username, event_title, event_shortdesc, event_start) VALUES('" . $_SESSION['username'] . "', '" . $safe_event_title . "', '" . $safe_event_shortdesc . "', '" . $event_date . "')";
  ...
  $getEvent_sql = "SELECT event_title, event_shortdesc, date_format(event_start, '%l:%i %p') as fmt_date FROM calendar_events WHERE username = '" . $_SESSION['username'] . "' AND month(event_start) = '" . $safe_m . "' AND dayofmonth(event_start) = '" . $safe_d . "' AND year(event_start) = '" . $safe_y . "' ORDER BY event_start";
  ...
  echo <<<END_OF_TEXT
  <form method="post" action="$_SERVER[PHP_SELF]" style="width: 70%">
