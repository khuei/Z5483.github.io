...
let queryURL = "https://api.artic.edu/api/v1";
let limitQuery = 100;
...
function loadCurrentView() {
    currentArtwork.innerHTML = '<input type="hidden" id="current-artwork-value" name="artwork-data" value="default_value">';
    ...
    let addCollectionButton = document.createElement("button");
    addCollectionButton.setAttribute("id", "add-collection-button");
    addCollectionButton.textContent = "Add To Collection";
    ...
    addCollectionButton.addEventListener("click", () => {
        ...
        // Get UTC components (year, month, day, hours, minutes, seconds)
        var year = currentUtcDate.getUTCFullYear();
        var month = currentUtcDate.getUTCMonth() + 1; // Months are zero-indexed, so add 1
        var day = currentUtcDate.getUTCDate();
        ...
        currentArtworkJSON.datetime_added = formattedUtcTime;

        currentArtworkJSON.username = currentUsername;

        document.getElementById("current-artwork-value").value = JSON.stringify(currentArtworkJSON);
        currentArtwork.submit();
    });

    // Append elements to the current artwork div
    buttonDiv.appendChild(addCollectionButton);

    if (typeof isLoggedIn !== 'undefined' && isLoggedIn) {
        $(addCollectionButton).fadeIn(0);
    } else {
        $(addCollectionButton).fadeOut(0);
    }
...
function loadGallery() {
    ...
    $.ajax({
        url: queryURL + "/artworks?limit=" + limitQuery +
             "&fields=image_id," +
             "title," +
             "artist_title," +
             "artist_display," +
        ...
        success: function (data) {
            ...
            artworkJSON = data.data;
            imageBaseURL = data.config.iiif_url;
